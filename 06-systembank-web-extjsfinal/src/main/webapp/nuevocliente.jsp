...
<body>
	...
	<form action="guardar" method="post">
		<input type="hidden" name="idCliente"
			value="<jsp:getProperty property="idCliente" name="cliente" />" />
		...
		<select name="idBanco">
			<c:forEach items="${bancos}" var="banco">
				<c:choose>
					<c:when test="${banco.getIdBanco() eq cliente.getIdBanco()}">
						<option selected value="${banco.getIdBanco()}">${banco.getNombre()}</option>
					</c:when>
					<c:otherwise>
